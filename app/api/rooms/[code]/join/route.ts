...
export async function POST(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const body = (await request.json()) as { displayName?: unknown; isCoHostInvite?: unknown };
  const displayName = typeof body.displayName === "string" ? body.displayName.trim().slice(0, 60) : "Guest";
  const isCoHostInvite = body.isCoHostInvite === true;
  ...
  const isHost = Boolean(isCoHostInvite && attendeeEmail && (room.host_email.toLowerCase() === attendeeEmail || room.hosts.some((host) => host.email.toLowerCase() === attendeeEmail)));
  ...
  response.cookies.set(`bh_room_access_${code}`, isHost ? "host" : "guest", { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 60 * 60 * 12 });
  return response;
}