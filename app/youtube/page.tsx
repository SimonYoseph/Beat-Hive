...
function QueueTrackItem({ track, index, isPlaying, canReorder, canRemove, canControlPlayback, showUpvoteCount, onRemove, onUpvote, onPlayNow, onMoveToTop }: QueueTrackItemProps) {
  ...
  return (
    <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition: isDragging ? undefined : transition }} className={`flex min-w-0 items-center gap-2 rounded-lg bg-[#1b1b1b] p-2 will-change-transform sm:gap-3 sm:p-3 ${isDragging ? "opacity-40" : ""}`}>
      <span className="w-4 text-center text-xs font-bold text-yellow-500 sm:w-5 sm:text-sm">{index + 1}</span>
      {track.thumbnail && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={track.thumbnail} alt="" className="h-10 w-14 shrink-0 rounded object-cover sm:h-12 sm:w-16" />
      )}
      ...
      <div className="flex max-w-[128px] shrink-0 flex-col items-center gap-1 sm:max-w-[170px]">
