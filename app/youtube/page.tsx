    ...
    <main className="min-h-[100dvh] bg-[#111] px-4 py-12 text-white sm:px-6">
      <section className="mx-auto max-w-2xl">
        ...
        <section className="mt-6">
          ...
          {!isHistoryCollapsed && (playedTracks.length === 0 ? <p className="text-center text-sm text-gray-500">Played songs from Hive Queue will appear here.</p> : <div className="space-y-2">{[...playedTracks].reverse().slice(0, 10).map((track, index) => <article key={`${track.videoId}-${index}`} className="flex items-center gap-3 rounded-lg bg-[#1b1b1b] p-3"><span className="w-5 text-center text-sm font-bold text-gray-500">{playedTracks.length - index}</span><div className="min-w-0 flex-1"><h3 className="truncate font-bold">{track.title}</h3><p className="truncate text-sm text-gray-400">{track.channelTitle}</p></div></article>)}</div>)}
        </section>
